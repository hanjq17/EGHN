...
class Complex(PhysicalObject):
    def __init__(self, n_balls, node_idx, charge, type):
        super().__init__(n_balls, node_idx, charge, type)
        self.xc, self.vc, self.wc = None, None, None
        self.length = None

    def initialize(self, X, V, X_c, rr):
        x, v = X[self.node_idx], V[self.node_idx]  # [N, 3]
        if x.shape[0] == 1:
            # isolated particles
            return X, V

        # adjust the positions
        # xc = x[np.random.choice(np.arange(x.shape[0]), size=1, replace=False)]  # [1, 3]
        rand_r = np.random.randn(x.shape[0], x.shape[1])  # [N, 3]
        rand_d = (np.random.rand(x.shape[0], 1) / 2 + 0.6) * rr  # [N, 1]
        rand_r = rand_r / np.sqrt(np.sum(rand_r ** 2, axis=-1).reshape(-1, 1)) * rand_d  # [N, 3]
        x = X_c + rand_r

        m = np.ones(x.shape[0]).reshape(-1, 1)  # [N,]
        xc = np.sum(x * m, axis=0) / np.sum(m)  # [3,]
        rc = x - xc  # [N, 3]
        vc = np.sum(v * m, axis=0) / np.sum(m)  # [3,]
        relative_v = v - vc  # [N, 3]
        w = np.cross(rc, relative_v) / np.sum(rc ** 2, axis=-1).reshape(-1, 1)  # [N, 3]
        # pooling over w
        wc = np.mean(w, axis=0)  # [3,]
        self.xc, self.vc, self.wc = xc, vc, wc  # [3,]
        ds = np.sqrt(np.sum(rc ** 2, axis=-1))  # [N,]
        self.length = ds
        new_v = np.cross(wc, rc)  # [N, 3]
        new_x = x  # [N, 3]
        X[self.node_idx] = new_x
        V[self.node_idx] = new_v
    ...
    def update(self, X, V, F, delta_t):
        ...
        if x.shape[0] == 1:
            ...
            V[self.node_idx] = v
            return X, V
        m = np.ones(x.shape[0])  # [N,]
        xc, vc, wc = self.xc, self.vc, self.wc  # [3,]
        r = x - xc  # [N, 3]
        # I did some modification here
        ac = np.mean(f, axis=0) / np.sum(m)  # [3,]
        # ac = np.mean(f, axis=0)
        vc = vc + ac * delta_t
        xc = xc + vc * delta_t

        # I did some modification here
        # J = np.sum(np.sum(r ** 2, axis=-1) * m, axis=0)  # [3,]
        temp = np.repeat(wc.T.reshape(1, -1), repeats=r.shape[0], axis=0)
        r_pro = (r * temp).sum(axis=-1) / (wc ** 2).sum()
        r_pro = r_pro.reshape(-1, 1) * np.repeat(wc.reshape(1, -1), repeats=r.shape[0], axis=0)
        r_d = r - r_pro
        J = np.sum(np.mean(r_d ** 2, axis=-1), axis=0)
        M = np.sum(np.cross(r, f), axis=0)  # [3,]
        beta = M / J  # [3,]
        ...
        M = get_rotation_matrix(theta, wc / wc_norm)  # [3, 3]
        _r = np.matmul(M, r.T).T  # [N, 3]
        new_x = xc + _r  # [N, 3]
        new_v = vc + np.cross(wc.reshape(1, -1), _r)  # [N, 3]
    ...
    def check(self, X, V):
        ...
        rc = x - xc  # [N, 3]
        ds = np.sqrt(np.sum(rc ** 2, axis=-1))  # [N,]
        assert np.sum(np.abs(ds - self.length)) < eps
